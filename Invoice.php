...
class Invoice extends MX_Controller {
    ...
    function bdtask_invoice_form() {
		...
		$data['companies'] = $this->invoice_model->getAllCompanies(); // irfan company Fetch all companies
        $data['module']        = "invoice";
    ...
    public function bdtask_invoice_details($invoice_id = null){
		...
		'company_id'        => $invoice_detail[0]['company_id'],
        ...
        'is_serial'         => $isserial,
        'is_unit'           => $isunit,
		 'companiesarray'       => $this->invoice_model->getAllCompaniesarray(), // Company data
        );
    ...
    public function bdtask_manual_sales_insert(){
        ...
        $company_id = $this->input->post('company_id', true);  // Get company_id from the form
        ...
        if($finyear<=0){
            ...
            if ($this->form_validation->run() === true) {
				...
				$invoice_id = $this->invoice_model->invoice_entry($incremented_id, $company_id);  // Pass company_id here 
                ...
                if(!empty($invoice_id)){
                    ...
                    if($normal == 1){
                        $printdata       = $this->invoice_model->bdtask_invoice_pos_print_direct_sale($invoice_id);
                        $data['details'] = $this->load->view('invoice/invoice_sale_html', $printdata, true);
						 log_message('debug', 'Invoice HTML Manual: ' . $data['details']); // Debug added

                    }else{
                        $printdata       = $this->invoice_model->bdtask_invoice_pos_print_direct_sale($invoice_id);
                        $data['details'] = $this->load->view('invoice/invoice_sale_html', $printdata, true);
						log_message('debug', 'Invoice POS Print: ' . $data['details']); // Debug added
                    }
					
					
					                   // Call the bdtask_invoice_details function and store the result in details
             //   ob_start(); // Buffering output
               // $this->bdtask_invoice_details($invoice_id); // Directly call the function with $invoice_id
                //$data['details'] = ob_get_clean(); // Store the output in details
    ...
    public function autoapprove($invoice_id){
        ...
        $vouchers = $this->db->select('referenceNo, VNo, company_id')->from('acc_vaucher')->where('referenceNo',$invoice_id)->where('status',0)->get()->result();
        foreach ($vouchers as $value) {
            # code...
            $data = $this->Accounts_model->approved_vaucher($value->VNo, 'active', $value->company_id);
        }
        return true;
  ...
  public function bdtask_autocomplete_product(){
     ...
     /*product autocomple search  irfan*/
       /* public function bdtask_autocomplete_product(){
        $product_name   = $this->input->post('product_name',TRUE);
		 $company_id   = $this->input->post('company_id',TRUE);
        $product_info   = $this->invoice_model->autocompletproductdata($product_name, $company_id);
       if(!empty($product_info)){
        $list[''] = '';
        foreach ($product_info as $value) {
            $json_product[] = array('label'=>$value['product_name'].'('.$value['product_model'].')','value'=>$value['product_id']);
